...
export default function CategoryPage({ config }: { config: SiteConfig }) {
  ...
  const activeCategoryLabel = categoryId ? (categoryMap[categoryId] || (currentBrand ? "Điều Hòa" : categoryId)) : "";
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {sectionProducts.map((product) => (
                              <div key={product.id} className="bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(26,26,46,0.09)] hover:shadow-[0_8px_28px_rgba(200,16,46,0.14)] hover:-translate-y-1 transition-all duration-300 flex flex-col group">
                                <div className="relative aspect-square mb-4 bg-[#f0f0f8] rounded-xl overflow-hidden p-4 flex items-center justify-center">
                                  <EditableImage 
                                    id={`prod_img_${product.id}`}
                                    defaultSrc={product.image}
                                    className="max-h-[140px] w-auto object-contain group-hover:scale-110 transition-transform duration-500" 
                                    alt={product.name} 
                                  />
                                  <span className="absolute top-3 left-3 bg-[#C8102E] text-white text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-tighter z-10">
                                    Giảm 20%
                                  </span>
                                </div>
                          ...
                          <div className="flex-1 space-y-6">
                            <div className="space-y-4">
                              ...
                              <div className="space-y-2">
                                 ...
                                 <p className={`${categoryId === "bo-nang-luong-mat-troi" ? "text-blue-600" : "text-red-600"} font-bold text-sm uppercase tracking-widest`}>
