...
import { MessageCircle, X, Send, Bot, User as UserIcon, Loader2, Image as ImageIcon, Trash2 } from "lucide-react";
...
export default function ChatBot({ config }: { config: SiteConfig }) {
  ...
  const fetchSheetContext = async (token: string) => {
    try {
      ...
      console.error("Failed to fetch sheet context for chatbot");
    }
  };
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-8 bg-slate-50/50">
              ...
              {messages.map((msg, i) => (
                ...
                >
                  ...
                  <div className={`max-w-[80%] p-5 rounded-3xl text-sm shadow-sm leading-relaxed ${
                    msg.role === "user" 
